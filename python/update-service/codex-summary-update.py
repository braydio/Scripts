...
class Ctx:
    ...
    def run(self, cmd, sudo: bool = False, allow_fail: bool = False) -> int:
        ...
        try:
            cp = subprocess.run(cmd, check=not allow_fail)
            return cp.returncode
        except subprocess.CalledProcessError as e:
            self.log(style(f"Command failed {e.returncode}: {cmd_s}", C.RED))
            if allow_fail:
                return e.returncode
            raise
...
def build_summary_prompt(log_text: str) -> str | None:
    ...
    - Note if reboot or service restarts appear needed
...
def main():
    ...
    # log directory
    log_dir = Path(args.log_dir or os.environ.get("UPDATE_LOG_DIR", "logs"))
